...
class dbInsertNuovi():
    def __init__(self,name='alboPretorio'):
        ...
        self.cur.execute('''CREATE TABLE IF NOT EXISTS alboPretorio_files (
                      ...
                      path TEXT NOT NULL,
                      PRIMARY KEY (id_registro_anno, id_registro_num, path)
                      );''')
	...
	try:
		...
		if count == 0:
		    ...
		    self.db.commit()
		res = self.cur.execute('UPDATE alboPretorio SET age=0 WHERE id_registro_anno = ? AND id_registro_num = ?', (item['id_registro_anno'],item['id_registro_num']))
	        self.db.commit()
	except Exception, e:
		print "Exception %s"%(e,)
		pass
	...
	if len(item['files']) > 0:
		for f in item['files']:
			res = self.cur.execute('SELECT count(*) FROM alboPretorio_files WHERE id_registro_anno = ? AND id_registro_num = ? AND path = ?', (item['id_registro_anno'],item['id_registro_num'],f['path']))
			count = res.fetchone()[0]
			if count == 0:
...
class dbElaboraNuovi():
    ...
    def do_tweet(self,id_registro_num,id_registro_anno,oggetto,link_dettaglio):
    ...
    def do_telegram(self,id_registro_num,id_registro_anno,oggetto,link_dettaglio):
        curFiles = self.db.cursor()
	tgLine = '[%s/%s] %s %s' % (str(id_registro_num), str(id_registro_anno), oggetto, link_dettaglio)
	try:
	    ...
	    resFiles  = curFiles.execute('SELECT titolo, path FROM alboPretorio_files WHERE id_registro_anno = ? AND id_registro_num = ?', (id_registro_anno, id_registro_num))
    ...
    def elabora(self):
	    self.curWrite = self.db.cursor()
            res = self.cur.execute('SELECT id_registro_num, id_registro_anno,oggetto,data_inizio_pub,data_fine_pub,link_dettaglio FROM alboPretorio WHERE NOT tweeted ORDER BY id_registro_anno, id_registro_num')
            rows=self.cur.fetchall()
            for row in rows:
                (id_registro_num, id_registro_anno,oggetto,data_inizio_pub,data_fine_pub,link_dettaglio) = row
		self.do_tweet(id_registro_num, id_registro_anno, oggetto,link_dettaglio)

            res = self.cur.execute('SELECT id_registro_num, id_registro_anno,oggetto,data_inizio_pub,data_fine_pub,link_dettaglio FROM alboPretorio WHERE NOT telegrammed ORDER BY id_registro_anno, id_registro_num')
            rows=self.cur.fetchall()
            for row in rows:
		(id_registro_num, id_registro_anno,oggetto,data_inizio_pub,data_fine_pub,link_dettaglio) = row
		self.do_telegram(id_registro_num, id_registro_anno, oggetto, link_dettaglio)
